<template>
  <div>
        ...
        <!-- 项目经验 -->
        <div class="creative_experience" v-popover:editProjectExperience>
            <h2 class="creative_experience-title"><span>项目经验</span><hr/></h2>
            <ul class="creative_experience-content" v-for="(item, index) in ProjectExperience" :key="index">
              <li class="creative_experience-content-title"><span>{{item.project}}</span>|<span>{{item.time && item.time.join('-')}} </span><span>{{item.responsibility}}</span></li>
              <li class="creative_experience-content-info" v-for="(_describe, index) in item.describe" :key="index">{{index+1}}、{{_describe.info}}</li>
            </ul>
        </div>
        <!-- End 项目经验 -->
  </div>
