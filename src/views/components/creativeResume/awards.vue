<template>
  <div class="awards">
        <!-- 编辑荣誉奖项 -->
        <el-popover ref="editAwards" placement="bottom" width="500" trigger="click" @hide="endEditAwards">
            <div>
                <el-button @click="addDescribe(Awards)">添加个人信息</el-button>
            </div>
            <el-row>
                <el-col :span="24">
                    <el-table :data="Awards" highlight-current-row border style="width: 100%" @keyup.tab.native="addDescribe(Awards)">
        ...
        <!-- 荣誉奖项 -->
        <div class="awards-info-block" v-popover:editAwards>
            <h2 class="info-title">荣誉奖项</h2>
            <ul class="info-container" v-for="(item, index) in Awards" :key="index">
                <li><span>{{item.label}}：</span><span>{{item.info}}</span></li>
            </ul>
        </div>
        <!-- End 荣誉奖项 -->
  </div>
...
export default {
  ...
  methods: {
    //  完成编辑并保存所有编辑状态
    endEditAwards() {
      this.Awards = this.Awards.filter(_describe => {
