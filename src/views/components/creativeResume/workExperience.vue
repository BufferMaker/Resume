<template>
  <div>
        ...
        <!-- 工作经验 -->
        <div class="creative_experience" v-popover:editWorkExperience>
            <h2 class="creative_experience-title"><span>工作经验</span><hr/></h2>
            <ul class="creative_experience-content" v-for="(item, index) in workExperience" :key="index">
              <li class="creative_experience-content-title"><span>{{item.company}}</span>|<span> {{item.time && item.time.join('-')}} </span><span>{{item.position}}</span></li>
