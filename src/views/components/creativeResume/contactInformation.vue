<template>
  <div class="contact-info">
        <!-- 编辑联系方式 -->
        <el-popover ref="editContact" placement="bottom" width="500" trigger="click" @hide="endEditContact">
            <div>
                <el-button @click="addDescribe(Contact)">添加个人信息</el-button>
            </div>
            <el-row>
                <el-col :span="24">
                    <el-table :data="Contact" highlight-current-row border style="width: 100%" @keyup.tab.native="addDescribe(Contact)">
        ...
        <!-- 联系方式 -->
        <div class="contact-info-block" v-popover:editContact>
            <h2 class="info-title">联系方式</h2>
            <ul class="info-container" v-for="(item, index) in Contact" :key="index">
...
export default {
  ...
  methods: {
    //  完成编辑并保存所有编辑状态
    endEditContact() {
      this.Contact = this.Contact.filter(_describe => {
